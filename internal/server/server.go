...
func NewServer(
	...
	logger := slog.Default()
	healthService := app.NewHealthService(logger, db)
	mux.Handle("GET /", handler.Index(providers))
	mux.Handle("GET /static/", http.StripPrefix("/static/", http.FileServer(http.FS(static.FS))))
	mux.Handle("GET /livez", handler.Live(healthService))
	mux.Handle("GET /readyz", handler.Ready(healthService))
	mux.Handle("GET /metrics", middleware.MetricsAuth(cfg.MetricsToken)(promhttp.HandlerFor(reg, promhttp.HandlerOpts{
		Registry: reg,
	})))

	authService := app.NewAuthService(logger, db, nil) // TODO: pass app.Providers adapted from provider.Providers
